...
class Macro(Statement):
    ...
    def parse_next(self, data):
        ...
        if data[0] == "CLASS":
            ...
            width = float(data[1])
            height = float(data[3])
            self.info["SIZE"] = str(width) + " " + str(height)
            lef.append(("Size " + str(self.info["SIZE"])))
            ...
            #print(str(self.pin_dict[data[1]]))
            if "PIN" in self.info:
                self.info["PIN"].append(new_pin)
                #print(self.info["PIN"][0])
            else:
...
class Rect:
    ...
    def __init__(self, points):
        ...
        self.type = "RECT"
        self.points = points
        lef.append(("RECT "+str(self.points)))
