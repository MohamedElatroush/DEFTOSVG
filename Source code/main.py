...
pth=""
dotindx=""
read_path=input("Please enter DEF file path: ")
pth = read_path.split("/")
...
path=input("Please enter LEF file path: ")
            ...
            if MACRO[k] ==lef_info[j]:                           #Matching macros from LEF / DEF
                ...
                for m in range(j,endindx):
                    if (lef_info[m]=="Size"):
                        ...
                        if (orientation=="N"):
                            d.append(draw.Rectangle(xplacement-dx0+x0 , -(height-(yplacement-dy0+y0)), rwidth, rheight, fill=color, fill_opacity=opacity, stroke_width=1,stroke='black'))
...
d.saveSvg(SVG)         #draw svg image and give it a file name
d  # Display as SVG